...
public class MainInt implements ClientModInitializer {
	public static List<String> options = new ArrayList<>(List.of("Distance", "Bps", "Ping", "Health", "DamageSource", "Team", "Air", "Gamemode", "Yaw", "Vehicle", "Armor", "Name", "UseTime", "HurtTime", "Pitch", "Arm", "Age"));
	@Override
	...
	public static  MutableText colored(String str, String preset, LivingEntity ent){
		...
		switch (preset){
			...
			case "team" -> altRGB = ent.getScoreboardTeam() != null && ent.getScoreboardTeam().getColor().getColorValue() != null ? ent.getScoreboardTeam().getColor().getColorValue() : 0;
			case "hurtTime" ->{
				r = 255;
	...
	public static Text getText(Entity entity){
		...
		if(entity instanceof LivingEntity ent) {
			...
			fields.put("ping", "0");
			fields.put("gamemode", "");
			if (ent instanceof PlayerEntity enty) {

				try {
					fields.put("ping", String.valueOf(mc.getNetworkHandler().getPlayerListEntry((enty).getGameProfile().getName()).getLatency()));
			...
			fields.put("team", ent.getScoreboardTeam() != null ? "Team: ".concat(ent.getScoreboardTeam().getName()) : "");
